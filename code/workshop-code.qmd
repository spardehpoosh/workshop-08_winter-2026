...
[deployed html url](https://spardehpoosh.github.io/workshop-08_winter-2026/code/workshop-code.html)
...
drought_exp_clean <- drought_exp |> 
  ...
  clean_names() |> 
  # created a new column called species_name and relabelled all species with full name
  ...
  # created a new column for treatment and changing abbreviations to corresponding full names
  mutate(water_treatment = case_match(
    water,
    "WW" ~ "Well watered",
    "DS" ~ "Drought stressed"
  ))  |> 
  # making sure that categorical variables are factors, and seeing factor level order
...
```{r}
#| label: scatterplot visualization

# base layer: ggplot
ggplot(data = drought_exp_clean, # using salinity data set for plot
       aes(x = sla, # x-axis predictor - soil salinity
           y = total_g)) + # y-axis response - California pickleweed biomass
  geom_point(color = "orchid4") + # changing color of individual points
  # relabelling axes
  labs(x = "Specific Leaf Area (cm\U00B2/g)", 
       y = "Total Mass (g)",
       title = "As SLA increases, total mass (g) increases") # adding title
...
```{r}
#| label: jitterplot for water treatments

# base layer: ggplot call
ggplot(data = drought_exp_clean, # starting with maples_2003 data frame
       aes(x = water_treatment, # watershed on the x-axis
           y = total_g, # stem length on the y-axis 
           color = water_treatment)) + # coloring by watershed
  ...
  labs(x = "Water Treatment", # relabelling x axis
       y = "Total Mass (g)", # relabelling y axis
      title = "On average, plant mass in well watered treatment is higher than plant mass\nin drought stressed treatment") + 
...
```{r}
#| label: jitterplot for species

# base layer: ggplot call
ggplot(data = drought_exp_clean, # starting with maples_2003 data frame
       aes(x = species_name, # watershed on the x-axis
           y = total_g, # stem length on the y-axis 
           color = species_name)) + # coloring by watershed
  geom_jitter(height = 0, # second layer: jitter to represent individual plants accross species
              width = 0.2) +
  ...
  # wrap axis along labels
  scale_x_discrete(labels = label_wrap(10)) +
  labs(x = "Species", # relabelling x axis
       y = "Total Mass (g)", # relabelling y axis
      title = "Plant mass differs accross species") + 
  theme(legend.position = "none") # removing legend
...
The best model to describe variation in plalnt mass only includes water treatment and species. We know this because it has the lowest AIC
...
Residuals (standing in for errors) look normally distributed based on qqplot. 
...
Drought stressed is the reference level for water treatment, and lotus scoparius is the reference level for species name. This is because lotus came first and drought stress came first at the top of factor relevel. 

**What does the `(Intercept)` represent?**  

The intercept is the model prediction for plant mass for lotus scoparius in the drought stressed treatment. The intercept represents the model estimate when all predictors are at the reference level. The model predicts that lotus scoparius mass in the drought stressed treatment will be 0.055 $\pm$ 0.025 (SE) g. 
...
This is the model prediction for the difference between Penstemon centranthifolius mass in the well watered treatment and Lotus scoparius mass in the well watered treatment. The model predicts that this difference is 0.05 $\pm$ 0.03 g. 

**What does `species_nameEschscholzia californica` represent?**

This is the model prediction for the difference between Eschsholzia californica mass in the weel watered treatment and Lotus scoparius mass in the well watered treament. The mode predicts that this difference is 0.23 $\pm$ 0.03 g. 
